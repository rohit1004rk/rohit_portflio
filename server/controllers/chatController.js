...
function getBotReply(input) {
  ...
  if (
    has(["who are you", "your name", "about you", "introduce", "about rohit"])
  ) {
  ...
  if (
    has(["skill", "tech", "stack", "language", "framework", "tool", "know"])
  ) {
    return `Here's Rohit's toolkit:\n\n${KNOWLEDGE.skills}`;
  }

  if (
    has(["education", "college", "degree", "study", "b.tech", "university"])
  ) {
  ...
  if (
    has(["ai", "ml", "machine learning", "nlp", "chatbot", "computer vision"])
  ) {
...
export const createChat = async (req, res) => {
  try {
    ...
    let reply = await generateChatReply({
      message,
      knowledge: KNOWLEDGE,
    });
...
export const getChatLogs = async (req, res) => {
  try {
    const logs = await ChatLog.find()
      .select("userMessage botReply sessionId userAgent createdAt updatedAt")
      .sort({ createdAt: -1 })
      .limit(200);
