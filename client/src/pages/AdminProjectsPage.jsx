...
function AdminProjectsPage() {
  ...
  const handleEdit = (project) => {
    ...
    setForm({
      ...
      displayNumber: project.displayNumber ?? 0,
    });
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      setOrderDirty(false);
      setSuccess("Project sequence saved successfully.");
    } catch (err) {
  ...
  const handleMove = async (projectId, direction) => {
    try {
      ...
      const endpoint =
        direction === "up"
          ? `/api/projects/${projectId}/move-up`
          : `/api/projects/${projectId}/move-down`;
      ...
      if (!response.ok) {
        throw new Error(data.message || `Failed to move project ${direction}.`);
      }

      const projectList = Array.isArray(data.projects) ? data.projects : null;
      ...
      setSuccess(
        direction === "up"
          ? "Project moved up successfully."
          : "Project moved down successfully.",
      );
    } catch (err) {
      setError(err.message || "Could not change project sequence.");
    } finally {
  ...
  const handleSwap = async (project) => {
    const targetSequence = window.prompt(
      `Swap "${project.title || "this project"}" with which sequence number?\n\nEnter the current sequence number of the target project.`,
      "",
    );
    ...
    const sequenceNumber = Number(targetSequence);

    if (!Number.isInteger(sequenceNumber) || sequenceNumber < 1) {
      setError("Please enter a valid positive sequence number.");
      return;
    }

    const targetProject = projects.find(
      (item) => Number(item.order) === sequenceNumber,
    );

    if (!targetProject) {
      setError(`No project found with sequence number ${sequenceNumber}.`);
      return;
    }

    if (targetProject._id === project._id) {
      setError("A project cannot be swapped with itself.");
      return;
    }

    const confirmed = window.confirm(
      `Swap "${project.title || "Project"}" with "${targetProject.title || "Project"}"?`,
    );
    ...
    try {
      ...
      const response = await fetch("/api/projects/swap", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to swap projects.");
      }
      ...
      setOrderDirty(false);
      setSuccess("Projects swapped successfully.");
    } catch (err) {
      setError(err.message || "Could not swap projects.");
    } finally {
      setActionSaving(false);
    }
  };
  ...
  const handleToggleEnabled = async (project) => {
    ...
    const confirmed = window.confirm(
      nextEnabled
        ? `Enable "${project.title || "this project"}"?`
        : `Disable "${project.title || "this project"}"?`,
    );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || "Failed to change project visibility.");
      }
      ...
      setProjects((current) =>
        current.map((item) =>
          item._id === project._id ? { ...item, ...updatedProject } : item,
        ),
      );
      ...
      setError(err.message || "Could not change project visibility.");
    } finally {
      setActionSaving(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const metricsToArray = (value) => {
        ...
        try {
          ...
          if (Array.isArray(parsed)) {
            return metricsToArray(parsed);
          }
        } catch {
          // Not JSON. Parse normal textarea lines below.
        }
      ...
      const currentThumbnail = formValue("thumbnail");

      const originalThumbnail = originalProject
        ? cleanText(originalProject.thumbnail)
        : "";

      const thumbnailChanged = currentThumbnail !== originalThumbnail;
      ...
      const payload = {
        ...
        description: textToArray(form.description),
        problem: formValue("problem"),
        whatIBuilt: formValue("whatIBuilt"),
        result: formValue("result"),
        workflow: textToArray(form.workflow),
        limitations: textToArray(form.limitations),
        future: textToArray(form.future),
        metrics: metricsToArray(form.metrics),
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setProjects((current) =>
        current.filter((projectItem) => projectItem._id !== id),
      );
          ...
          <div className="admin-projects-container">
            <header className="projects-page-header">
              <div>
                ...
                <p>
                  Add, edit, delete, enable, disable and control project
                  sequence.
                </p>
              </div>
            ...
            <section className="projects-card">
              ...
              <form onSubmit={handleSubmit}>
                    ...
                    {basicFields.map((field) => (
                      ...
                      >
                        ...
                        {field.name === "displayNumber" && (
                          <small>
                            Display number is independent from sequence.
                          </small>
                        )}

                        {field.name === "order" && (
                          <small>
                            Sequence controls the actual project position.
                          </small>
                        )}
                      </div>
                    ))}
                ...
                <div className="project-control-grid">
                  ...
                  <div className="project-featured-box enabled-box">
                    <label className="featured-control">
                      <input
                        type="checkbox"
                        name="enabled"
                      ...
                      <span>
                        ...
                        <small>
                          Control whether this project is active on the
                          portfolio.
                        </small>
                      </span>
                    </label>
            ...
            <section className="projects-card existing-projects-card">
              <div className="projects-card-header existing-header">
                ...
                <div>
                  ...
                  <p>
                    ...
                    projects to change their sequence.
                  </p>
                </div>
                ...
                <div className="existing-projects-header-actions">
                  ...
                  <button
                    ...
                    disabled={!orderDirty || orderSaving || actionSaving}
                  >
                    {orderSaving ? (
                      <>
                        <span className="button-spinner" />
                        ...
                        >
                          ...
                          <td>
                            <span className="display-number">
                              {Number(project.displayNumber) > 0
                          ...
                          <td>
                            <div className="project-actions">
                              ...
                              <button
                                ...
                                onClick={() => handleMove(project._id, "up")}
                                disabled={actionSaving || index === 0}
                              ...
                              <button
                                type="button"
                                className="table-move-btn"
                                onClick={() => handleMove(project._id, "down")}
                                disabled={
                                  actionSaving || index === projects.length - 1
                                }
                              ...
                              <button
                                ...
                                disabled={actionSaving || projects.length < 2}
...
const adminProjectsStyles = `
  ...
  .table-enable-btn:disabled,
  .table-disable-btn:disabled,
  .table-move-btn:disabled,
  .table-swap-btn:disabled {
    opacity: 0.45;
    cursor: not-allowed;
  }
  ...
  .table-edit-btn:hover:not(:disabled) {
  ...
  .table-enable-btn:hover:not(:disabled) {
  ...
  .table-disable-btn:hover:not(:disabled) {
  ...
  .table-move-btn:hover:not(:disabled) {
    border-color: #2dd4bf;
    color: #5eead4;
  }
  ...
  .table-swap-btn:hover:not(:disabled) {
